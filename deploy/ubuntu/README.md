...
数据库每天自动备份到 `/var/backups/nova/database`，默认保留 14 天。应用更新前也会备份；新版本健康检查失败时自动回滚。

## 正式收款前

- 在后台完成 SMTP、支付接口、异步回调、Turnstile、站点条款和 Telegram 客服配置。
- 在真实 Ubuntu 主机完成“创建入站 → 创建套餐和用户 → 支付或手动开通 → 获取订阅 → 客户端连接 → 流量统计 → 到期、重置和续费”闭环验收。
- 支付回调必须使用 HTTPS；Cloudflare 或其他代理开启后仍要保证源站证书有效。
- 不要把内部监听端口暴露到公网；只放行 Nginx 的 80/443 和实际节点业务端口。
