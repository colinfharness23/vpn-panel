...
function bypassMigratedRoute(req) {
  ...
  if (url === basePath) return "/portal.html";

  if (url.startsWith(basePath)) {
    const stripped = url.slice(basePath.length);
    if (PORTAL_ROUTES.has(stripped.replace(/\/$/, ""))) {
      return "/portal.html";
    }
...
export default defineConfig({
    ...
    rollupOptions: {
      ...
      output: {
        manualChunks(id) {
          ...
          if (id.includes("/node_modules/persian-calendar-suite/"))
  ...
  server: {
    ...
    strictPort: true,
    proxy: {
      "^/(?:[^/]+/)?(login|logout|getTwoFactorEnable|csrf-token|panel|server)(?:/|$)":
